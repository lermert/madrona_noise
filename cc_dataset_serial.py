...
class CCDataset_serial(object):
    ...
    def plot_stacks(self, stacklevel=1, outfile=None, lag_max=20, scale_factor_plotting=0.1,
                    plot_mode="heatmap", lag_min=0.0, cmap=plt.cm.bone,
                    mask_gaps=False, step=None, figsize=None,
                    color_by_cc=False, normalize_all=False, label_style="month",
                    ax=None, plot_envelope=False, ref=None, cluster_label=None,
                    mark_17_quake=False, grid=True, marklags=[], colorful_traces=False):
        if mask_gaps and step == None:
            step = np.median(np.diff(self.dataset[stacklevel].timestamps))
            print(step)
            #raise ValueError("To mask the gaps, you must provide the step between successive windows.")

        if cluster_label is not None:
            to_plot = self.dataset[stacklevel].data[self.dataset[stacklevel].cluster_labels==cluster_label]
            t_to_plot = self.dataset[stacklevel].timestamps[self.dataset[stacklevel].cluster_labels==cluster_label]
        else:
        ...
        if plot_mode == "traces":            
            ...
            if not mask_gaps:
                dat_mat = np.zeros((to_plot.shape[0],
                                    self.dataset[stacklevel].npts))
        ...
        if lag_max - lag_min > 50:
            tickstep = 10.0
        elif lag_max - lag_min > 20:
            tickstep = 5.0
        elif lag_max - lag_min > 10:
            tickstep = 2.0
        else:
            tickstep = 1.0
        ax1.set_xticks([i for i in np.arange(lag_min, lag_max, tickstep)])
        ax1.set_xticklabels([str(i) for i in np.arange(lag_min,
                                                       lag_max, tickstep)])
